...
# ── 在线检测器 ─────────────────────────────────────────────────────────
class OD:
    POT_ENTER = -80; POT_EXIT = -15; POT_MIN_VOL = 0.5; POT_CONFIRM_VOL = 0.15
    def __init__(s, fr):
        s.fr = fr; s.va = 0.15; s.da = 0.05
        s.vs = None; s.pvs = None; s.pt = None; s.ds = 0.0; s.ps = "IDLE"
        s.pe = None; s.md = 0.0; s.cv = None; s.ev = None; s.pd = False
        s.vh = []; s.dh = []; s.sh = []
    def feed(s, t0, v0):
        v = t0 * s.fr
        if s.vs is None: s.vs = float(v0)
        else: s.vs = s.va * float(v0) + (1 - s.va) * s.vs
        if s.pt is not None and s.pvs is not None:
            dt = t0 - s.pt; dv = s.vs - s.pvs; dr = dv / dt if dt > 0 else 0
        else: dr = 0
        s.ds = s.da * dr + (1 - s.da) * s.ds
        s.pvs = s.vs; s.pt = t0
        if not s.pd and v > s.POT_MIN_VOL:
            if s.ps == "IDLE":
                if s.ds < s.POT_ENTER: s.ps = "TRACKING"; s.md = s.ds; s.cv = v; s.ev = v
            elif s.ps == "TRACKING":
                if s.ds < s.md: s.md = s.ds; s.cv = v
                if s.ds > s.POT_EXIT and (v - s.ev) > s.POT_CONFIRM_VOL:
                    s.pe = s.cv; s.ps = "END_CONFIRMED"; s.pd = True
        s.vh.append(v); s.dh.append(s.ds); s.sh.append(s.ps)

# ── 预计算完整 dV/dt（画背景用） ────────────────────────────────────
bd = np.zeros(N); vs = None; pvs = None; pt = None; ds = 0.0
for i in range(N):
    v0 = float(mp[i]); t0 = float(T[i])
    if vs is None: vs = v0
    else: vs = 0.15 * v0 + 0.85 * vs
    if pt is not None and pvs is not None:
        dt = t0 - pt; dv = vs - pvs; bd[i] = dv / dt if dt > 0 else 0
    else: bd[i] = 0
    pvs = vs; pt = t0; ds = 0.05 * bd[i] + 0.95 * ds; bd[i] = ds
...
for ax in (ax1, ax2):
    ax.set_xlim(0, 2.25); ax.grid(True, alpha=0.3)
ax1.set_ylim(29500, 34000); ax1.set_ylabel("Potential (LSB)")
ax2.set_ylim(-350, 50); ax2.set_ylabel("dV/dt (LSB/s)"); ax2.set_xlabel("Volume (mL)")
...
for ax in (ax1, ax2):
    ax.axvline(1.089, color="gray", ls=":", lw=1, alpha=0.35, label="Ground truth ~1.089")
...
for i in range(N):
    ...
    if (i+1) % stride != 0 and i != N-1:
        continue
    vs = np.array(det.vh); ds = np.array(det.dh); ss = np.array(det.sh)
    ...
    if det.pe is not None: ep.set_xdata([det.pe, det.pe])
    txt = f"Step {i+1}/{N}  Vol = {u[i]:.4f} mL\nState: {det.ps}  dV/dt = {det.ds:.1f}"
    if det.pe: txt += f"\nENDPOINT = {det.pe:.4f} mL"
    elif det.cv and det.ps != "IDLE": txt += f"\nCandidate = {det.cv:.4f} mL"
    tx.set_text(txt)
...
plt.ioff()
print(f"\n✅ 检测完成！终点 = {det.pe} mL")
fig.suptitle(f"Detection Complete — Endpoint = {det.pe:.4f} mL", fontsize=13, fontweight="bold")
fig.canvas.draw_idle()
plt.show(block=True)
